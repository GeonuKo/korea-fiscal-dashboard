...
def load_all_data():
    ...
    # 편의를 위해 2013-2025 주요 데이터를 리스트로 구조화
    years_ext = list(range(2013, 2026))
    tax_income = [201.9, 205.5, 217.9, 242.6, 265.4, 293.6, 293.5, 285.5, 344.1, 395.9, 344.1, 367.3, 382.4] # 국세 요약
    non_tax = [45.1, 44.5, 45.0, 52.3, 53.0, 55.0, 48.0, 50.0, 54.0, 60.0, 52.0, 55.0, 57.0] # 세외 요약
    fund_income = [150.0, 155.0, 163.0, 175.0, 185.0, 195.0, 210.0, 225.0, 245.0, 260.0, 235.0, 245.0, 272.8] # 기금 요약
    ...
    # 3. 주요 국세 항목 데이터 (2025 본예산 기준 예시 포함)
...
if menu == "메인 대시보드":
    ...
    with col2:
        # 3. 국세 vs 세외 vs 기금 비중 (Donut Chart)
    ...
    # 4. 주요 국세 3대장 추이 (Line Chart)
    st.subheader("4. 주요 국세 항목(소득·법인·부가세) 추이")
    # 메모리 데이터를 기반으로 한 시계열 재구성 (예시 수치 포함)
